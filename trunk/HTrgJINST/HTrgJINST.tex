...
and having an unbiased performance comparison for our own GPU implementation it would have been preferable to use the CPU implementation of the Hough Transform from the Intel Performance Primitives (IPP) library.  However, it was observed that the implementation in IPP is not parallelized and so to provide a more fair performance comparison we have developed our own Hough Transform implementation for the CPU.  This CPU implementation is parallelized using OpenMP threads and the performance as the number of threads increases is shown in Figure~\ref{fig:cpu_speedup} for two slightly different implementations of the Hough Transform.  The CPU used for the performance results is a quad core CPU but supports up to two threads per physical core using Simultaneous Multithreading (SMT), with the Intel implementation of SMT known as HyperThreading (HT).  SMT does not show any performance advantage for this application because the speedup actually drops as the number of threads exceeds the number of physical cores.  For the GPU implementation, performance results for the two most recent generations of NVIDIA GPUs are shown in Figure~\ref{fig:TimePerformance}.  Note that the GPU implementation is up to 100x faster than our best multithreaded CPU implementation.
...
\begin{center}
	...
	\end{center}
\end{minipage}
% Maybe we don't want to use this??
%\begin{minipage}[t]{4.9cm}
%\begin{center}
