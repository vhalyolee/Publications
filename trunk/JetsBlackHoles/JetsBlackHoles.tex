...
\abstract{Massively parallel computing architecture at the LHC could be the leap necessary 
to reach a time of new discoveries at the LHC post the Higgs like discovery era.
...
system and the quest for new physics at the LHC suggest the  trigger to be the first place to 
integrate Graphic Processor Units /Many Integrater Core cards in its commercial computer farms.

This cutting edge technology provides  not only the tool to accelerate existing algorithms but 
also the opportunity to develop new algorithms that select events that could have evaded detection
 previously. In this article we describe new algorithms to select prompt or non prompt  jets and 
black holes like objects in the silicon tracker.
}
